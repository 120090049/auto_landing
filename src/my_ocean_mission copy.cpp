...
const char* exec_state_name[] = {"APPROACH_GNSS", "APPROACH_FRONT_CAMERA", "TRACKING_AND_LANDING", "LOST_AND_ASCENDING", "LAND"};
...
void boat_det_cb(const prometheus_msgs::DetectionInfo::ConstPtr &msg)
{
    ...
    if(boat_det.num_regain > VISION_THRES)
    {
...
void FlyState_update(void)
{
	switch(FlyState)
	{
		...
		case PREPARE: //飞到目标高度	
			...
			{
				ROS_INFO("Reach to initial height and ready for detection!");
				FlyState = FLY;
			}
				...
				switch (exec_state)
				{
					...
					case APPROACH_GNSS:
					{
						...
						else if(landpad_det.is_detected)
						{
							...
							Change_state = true;
							ROS_INFO("Get the down-looking camera detection result and get to the TRACKING_AND_LANDING.");
							std_msgs::Bool switch_front;
					...
					case TRACKING_AND_LANDING: 
					{
						...
						// 进入LANGING与上锁模式
						if (landpad_det.Detection_info.position[2] <= 0.2) 
						{
							// 切换到LANDING模式
							mavros_msgs::SetMode landing_set_mode;		
							landing_set_mode.request.custom_mode = "AUTO.LAND";
					...
					case LOST_AND_ASCENDING:
					{
						...
						if(landpad_det.is_detected)
						{
							exec_state = TRACKING_AND_LANDING;
							lost_time = 0;
							ROS_INFO("Regain the Landing Pad" );
							Change_state = true;
							break;
						}   
