...
const char* exec_state_name[] = {"APPROACH_GNSS", "TRACKING", "LOST"};
...
void boat_det_cb(const prometheus_msgs::DetectionInfo::ConstPtr &msg)
{
	...
	boat_det.pos_body_frame[0] = - boat_det.yaw_error;
    boat_det.pos_body_frame[1] = 0;
    boat_det.pos_body_frame[2] = 0;
	
    // 机体系 -> 机体惯性系 (原点在机体的惯性系) (对无人机姿态进行解耦)
    boat_det.pos_body_enu_frame = R_Body_to_ENU * landpad_det.pos_body_frame;
		...
		case FLY:
			{
				switch (exec_state)
				{
					...
					case APPROACH_GNSS:
					{
						...
						// cout << "(" << vel_target[0] << ", " << vel_target[1] << ") and yaw = " << target_theta << endl; 
						...
						if(boat_det.is_detected)
						{
							// exec_state = TRACKING;
							// Change_state = true;
							cout << "Get the front camera detection result." <<endl;
							break;
						}
						break;
					}
